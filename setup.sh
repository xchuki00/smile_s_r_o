...
#
#docker exec ${apache_container_id} cp -r bagisto/* .
#docker exec ${apache_container_id} rm -rf bagisto
# installing composer dependencies inside container
docker exec -i ${apache_container_id} bash -c "composer install"

# moving `.env` file
docker cp .configs/.env ${apache_container_id}:/var/www/html/.env
docker cp .configs/.env.testing ${apache_container_id}:/var/www/html/.env.testing

# executing final commands
docker exec -i ${apache_container_id} bash -c "php artisan bagisto:install --skip-env-check --skip-admin-creation"
